...
```{r}
weekend_activity_data<-activity_data[activity_data$daytype=='weekend',]
weekend_mean_steps_per_interval<-aggregate(weekend_activity_data['steps'],weekend_activity_data['interval'],FUN=mean)
weekend_plot<-ggplot(data=weekend_mean_steps_per_interval)+
    expand_limits(y=225)+
    geom_line(aes(x=interval,y=steps))+
    ggtitle("weekend")

weekday_activity_data<-activity_data[activity_data$daytype=='weekday',]
weekday_mean_steps_per_interval<-aggregate(weekday_activity_data['steps'],weekday_activity_data['interval'],FUN=mean)
weekday_plot<-ggplot(data=weekday_mean_steps_per_interval)+
    expand_limits(y=225)+
    geom_line(aes(x=interval,y=steps))+
    ggtitle("weekday")
grid.arrange(weekend_plot,weekday_plot)
